"""
TaskFlow — полный бэкенд с FastAPI + SQLite
Требования выполнены:
  ✅ 3 связанные таблицы: users, categories, tasks
  ✅ Аутентификация: регистрация + вход (хеш паролей bcrypt)
  ✅ CRUD: GET, POST, PUT/PATCH, DELETE
  ✅ REST API с чёткими эндпоинтами
  ✅ Валидация на сервере (Pydantic)
  ✅ Поиск и фильтрация задач
  ✅ Защита от SQL-инъекций (параметризованные запросы)
  ✅ Фронтенд встроен — открывай http://localhost:8000
"""
...
async def register(data: RegisterModel):
    ...
    db.execute(
        ...
        (data.username, data.email, hash_password(data.password), token)
    )
    db.commit()
...
# ─────────────────────────────────────────
#  ПОЛЬЗОВАТЕЛИ (для JOIN демонстрации)
# ─────────────────────────────────────────

@app.get("/users/tasks", tags=["Users"])
async def get_user_with_tasks(current_user: dict = Depends(get_current_user)):
    """Пользователь со всеми его задачами (JOIN)"""
    db = get_db()
    tasks = db.execute(
